...
export const generateHandler: FastifyPluginAsyncZod = async (app) => {
  app.post(
    ...
    {
      schema: {
        summary: "Generate a webhook handler",
        tags: ["Webhooks"],
        body: z.object({
          webhooksId: z.array(z.string()),
        }),
        response: {
          201: z.object({
            code: z.string(),
    ...
    async (request, reply) => {
      ...
      const webhooksData = await db
        ...
        .from(webhooks)
        .where(inArray(webhooks.id, webhooksId));

      const webhooksBodies = webhooksData.map((wh) => wh.body).join("\n\n");
